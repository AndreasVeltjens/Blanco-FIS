...
if (isset($_POST['importXML'])){
	...
	{
	   global $stack;   
	   $stack[count($stack)-2][] = $stack[count($stack)-1];
	   array_pop($stack);
	}
...
<form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form1">
  <table width="730" border="0" cellpadding="0" cellspacing="1">
    <tr> 
      <td colspan="4"><strong>Import XML Interface</strong></td>
    </tr>
    <tr> 
...
$d=intval($i/2);
  ...
  ?>
    ...
    <tr> 
      <td colspan="2"><em><strong>Daten- XML Import</strong></em></td>
      <td width="58">&nbsp;</td>
      <td width="275">&nbsp;</td>
    </tr>
    <tr> 
      <td colspan="2">Datei wählen</td>
      <td width="58">&nbsp;</td>
      <td width="275"><input type="file" name="file"> <input name="importXML" type="submit" id="importXML" value="Importieren"></td>
    </tr>
    <tr>
      <td colspan="2">Import</td>
      <td>&nbsp;</td>
      <td><?php foreach ($stack[0] as $row){
	  foreach ($row as $rowar){
	  $i++;
		  if (is_array($rowar)){
		   	foreach ($rowar as $rowbr){
				$ii++;
				if (is_array($rowbr)){
					foreach ($rowbr as $rowcr){
					$iii++;
					echo "[$i][$ii][$iii]--+".$rowcr."<br>";
					}		  
				}else{
				$iii=0;
				echo "[$i][$ii]-+".$rowbr."<br>";
    ...
    <tr> 
      ...
      <td width="58">&nbsp;</td>
      <td width="275"><?php echo print_r($stack); ?></td>
    </tr>
    <td colspan="4"><hr></td>
    </tr>
    <tr> 
