...
$suchtext=" WHERE ( fehlermeldungen.fmid like '".substr($HTTP_POST_VARS["suchtext"],1,100)."' or fehlermeldungen.fmid like '".$HTTP_POST_VARS["suchtext"]."' OR fehlermeldungen.fm1notes like '%".$HTTP_POST_VARS["suchtext"]."%' OR fehlermeldungen.fmsn like '".$HTTP_POST_VARS["suchtext"]."' OR fehlermeldungen.fm11notes like '".$HTTP_POST_VARS["suchtext"]."' "." OR fehlermeldungen.fm12notes like '".$HTTP_POST_VARS["suchtext"]."' "." OR fehlermeldungen.debitorennummer like '".$HTTP_POST_VARS["suchtext"]."' "." OR fehlermeldungen.debitorennummer like '".$HTTP_POST_VARS["suchtext"]."') ";}
/* alte version 
$suchtext="and ( fehlermeldungen.fmid like '".$HTTP_POST_VARS["suchtext"]."' OR fehlermeldungen.fm1notes like '%".$HTTP_POST_VARS["suchtext"]."%' OR fehlermeldungen.fmsn like '".$HTTP_POST_VARS["suchtext"]."' OR fehlermeldungen.fm11notes like '".$HTTP_POST_VARS["suchtext"]."' "." OR fehlermeldungen.fm12notes like '".$HTTP_POST_VARS["suchtext"]."' ) ";}*/
else{$suchtext="";}
...
/* Aktion Tauber Statement in SQL-Where   : fehlermeldungen.fm1b=1 */
mysql_select_db($database_qsdatenbank, $qsdatenbank);
$query_rst2 = "SELECT * FROM fehlermeldungen  $suchtext  ORDER BY fehlermeldungen.fmid DESC";
...
if ((isset($HTTP_POST_VARS["mitteilung"])) ) {
$updateGoTo = "qmb1.php?url_user=".$row_rst1['id'];
  if (isset($HTTP_SERVER_VARS['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $HTTP_SERVER_VARS['QUERY_STRING'];
  }
...
$updateGoTo = "la41.php?url_user=".$row_rst1['id']."&url_fmid=".$HTTP_POST_VARS["hurl_fmid"]."&qmb=1";
  header(sprintf("Location: %s", $updateGoTo));
}else{$errtxt = "Aktion wählen. Bitte erneut versuchen.";}

if ((isset($HTTP_POST_VARS["new"])) ) {
$updateGoTo = "la40.php?url_user=".$row_rst1['id']."&qmb=1";
  header(sprintf("Location: %s", $updateGoTo));
}else{$errtxt = "Aktion wählen. Bitte erneut versuchen.";}
  ...
  <table width="728" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      ...
      <td width="332"> <div align="right"> 
		  ...
		  <img src="picture/ur.gif" width="16" height="16">
		  <input name="mitteilung" type="submit" id="mitteilung" value="Mitteilung">
          <img src="picture/b_newdb.png" width="16" height="16"> 
          <input name="new" type="submit" id="new" value="neue Retourenabwicklung">
        </div></td>
    </tr>
...
<p>&nbsp;<a href="<?php printf("%s?pageNum_rst2=%d%s", $currentPage, 0, $queryString_rst2); ?>">First</a> 
  ...
  - Retourenabwicklung: <?php echo ($startRow_rst2 + 1) ?> bis <?php echo min($startRow_rst2 + $maxRows_rst2, $totalRows_rst2) ?><font size="2" face="Arial, Helvetica, sans-serif"> </font> </p>
