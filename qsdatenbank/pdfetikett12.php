<?php require_once('Connections/qsdatenbank.php'); 
		...
		/* Blanco Layout 10x5 - MaterialNummer + Seriennummer + Fertigungsdatum + EAN  */
...
do {
		...
		$barcodepath="barcode/testbildEAN13.png";
	 	$pdf->Image($barcodepath,65,5,20,40,"","");
...
$pdf->SetFont('Helvetica','',8);
	$pdf->Text(65,5,"EAN13");
		...
		$barcodepath="barcode/testbild.png";
	 	$pdf->Image($barcodepath,185,5,20,40,"","");
 ...
 */		/* $pdf->Text(65,5,"Serien-Nr."); */
		...
		$pdf->Text(2,30,$row_rst4['Nummer']);
		$pdf->SetFont('Helvetica','',12);
		$pdf->Text(2,35,"Einlagerung: ".date("d-m-Y",time() ));
/* alternativ Lagerplatzbezeichnung */	
		if($alternative==1){ 	
			$pdf->Text(2,40,"Lagerplatz: ".$row_rst4['lagerplatzalternative']); 
		}elseif($alternative==2){ 	
			$pdf->Text(2,40,"Lagerplatz: ".$row_rst4['lagerplatzalternative2']); 
		}else{		
			$pdf->Text(2,40,"Lagerplatz: ".$row_rst4['lagerplatz']);  
		}
	 
		
		$pdf->Text(2,47,"Verpackungsgröße: ".$row_rst4['verpackung']); 
		$pdf->Text(2,52,"Wiederbeschaffungszeit: ".$row_rst4['wiederbeschaffungszeit']); 
		$pdf->Text(2,60,"Kontierung: ".utf8_decode($row_rst4['Kontierung'])); 
		$pdf->Text(2,65,"Warengruppe: ".$row_rst4['Gruppe']); 
	/* 	$pdf->Text(2,33,"SN ".$url_nummer); */
		
		$pdf->SetFont('Helvetica','',14);
		$pdf->ln(7);
		 $pdf->MultiCell(160,4,utf8_decode(substr($row_rst4['Bezeichnung'],0,80)),0,"L",0); 
...
/* logo anzeigen */
		...
		if ($row_rst4['link_dokument']=="")
			{$bilddatei=$row_rst4['Nummer'].".png";}
		else 
			{$bilddatei=$row_rst4['link_dokument'];}
		
		
		$materialbild="..\\documents\materialbilder\\".$bilddatei;
		$pdf->Image($materialbild,100,15,80,55,"","");
		$pdf->Text(2,100,"_______________________________________________________________________________________________________________________________________________________________________________________________"); 
