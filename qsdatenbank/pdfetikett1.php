...
$totalRows_rst6 = mysql_num_rows($rst6);
		...
		$pdf->text(5,5, '                             Warenbegleitschein für eine Reparatur');
		$pdf->SetFont('Helvetica','',8);
		$pdf->Image('picture\logo.jpg',2,0,20,6,"","");
		$url_nummer=$row_rst1[fmid];
		if (strlen($url_nummer)==1){		
			$url_nummer="".$url_nummer;}
			elseif (strlen($url_nummer)==3){		
			$url_nummer="".$url_nummer;}
			elseif (strlen($url_nummer)==5){		
			$url_nummer="".$url_nummer;}
			elseif (strlen($url_nummer)==7){		
			$url_nummer="".$url_nummer;}
			elseif (strlen($url_nummer)==9){		
			$url_nummer="".$url_nummer;}
			else{
			$url_nummer="0".$url_nummer;
		}	
 include('picture/barcodeI25.php');
	 $pdf->Image('barcode\testbild.png',85,5,12,30,"","");
/*	 $url_nummer=$row_rst1[fmsn];
	 ...
	 if (strlen($url_nummer)==1){		
			...
			$url_nummer="00".$url_nummer;
		}
	 
	 	
include('picture/barcodeI25_0.php');
	 $pdf->Image('barcode\testbild2.png',95,15,12,30,"",""); */
		$pdf->SetFont('Helvetica','',8);
		$pdf->Text(2,10,'Fehlermeldenummer:');
		$pdf->SetFont('Helvetica','B',30);
		$pdf->Text(50,15,$row_rst1['fmid']);
		$pdf->SetFont('Helvetica','',8);
		$pdf->Text(2,15,'Leipzig, '.date("d.m.Y",time()));
		
		$pdf->SetFont('Helvetica','',12);
		
		$pdf->Text(2,20, substr(utf8_decode($row_rst1['rechnung']),0,30));
		$pdf->Ln(3);
		$pdf->SetFont('Helvetica','',7);
	if ((strlen($row_rst1['ansprechpartner']))>1){
		$pdf->Text(2,25,utf8_decode($row_rst1['ansprechpartner']));
		$pdf->Ln(3);
		}
		...
		$pdf->Text(55,30,utf8_decode($row_rst1['fmsn']));
		$pdf->SetFont('Helvetica','',7);
		$pdf->Ln(3);
	...
	*/
	...
	/* Fehlerfassung wird nicht mehr gedruckt.
