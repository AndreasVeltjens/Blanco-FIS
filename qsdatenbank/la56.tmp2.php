...
$query_rst2 = "SELECT artikeldaten.artikelid, artikeldaten.bezeichnung,
						date_format(fehlermeldungen.fm5datum,\"%m/%y\") as fehlerdatum, 
						count(fehlermeldungen.fmid) as anzahl,
						avg(datediff(fehlermeldungen.fm5datum,fehlermeldungen.fm3datum)) as mittelwert 
						FROM artikeldaten, fehlermeldungen 
						WHERE artikeldaten.artikelid=fehlermeldungen.fmartikelid $suchkrit 
...
mysql_select_db($database_qsdatenbank, $qsdatenbank);
$query_rst3 = "SELECT artikeldaten.artikelid, artikeldaten.bezeichnung,
				date_format(fehlermeldungen.fm5datum,\"%m/%y\") as fehlerdatum, 
				count(fehlermeldungen.fmid) as anzahl,avg(datediff(fehlermeldungen.fm5datum,fehlermeldungen.fm3datum)) as mittelwert, 
				entscheidung.entid, entscheidung.lokz, entscheidung.gvwl 
				FROM artikeldaten, fehlermeldungen, entscheidung 
				WHERE artikeldaten.artikelid=fehlermeldungen.fmartikelid $suchkrit 
				AND entscheidung.gvwl=1 AND entscheidung.entid =fehlermeldungen.fm2variante 
				GROUP BY date_format(fehlermeldungen.fm5datum,\"%Y %M\") ORDER BY fehlermeldungen.fm5datum desc limit 0,$limit";
...
$l2datay[$i]=round($row_rst3['anzahl'],1);
$i=$i-1;
...
$cplot->SetLegend(("Anzahl der Retoureneingang pro Monat"));
...
$bplot->SetLegend(("Anzahl Gewährleistung"));
// Display the values on top of each bar
$bplot->SetShadow();
...
$graph->yaxis->title->Set( ("Anzahl der Retoureneingänge"));
