...
mysql_select_db($database_qsdatenbank, $qsdatenbank);
$query_rst2 = "SELECT fhm_gruppe.id_fhm_gruppe, fhm_gruppe.piclink, fhm.id_fhm, fhm.id_fhm_gruppe, fhm.name, fhm.lokz, fhm.ortsveraend, fhm.still, fhm.lagerort, 
fhm.lastmod, fhm.Inventarnummer, fhm.werk, fhm_gruppe.funktion, fhm.sn,fhm.suchgruppierung, fhm.artikelid FROM fhm , fhm_gruppe 
WHERE fhm.id_fhm_gruppe=fhm_gruppe.id_fhm_gruppe $suchwhere AND fhm_gruppe.funktion=1 ORDER BY fhm.artikelid,fhm.sn";
  ...
  <table width="980" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
    ...
    <tr> 
      ...
      <td width="324"> <div align="right"> 
          ...
          <input name="new" type="submit" id="new" value="neues FHM anlegen">
        </div></td>
    </tr>
    <tr> 
      <td>FHM Gruppe</td>
...
?>
    ...
    <tr> 
      ...
      <td colspan="2"><input name="suchtext" type="text" id="suchtext" value="<?php echo $HTTP_POST_VARS['suchtext']; ?>" size="50"> 
        ...
        - Serviceger&auml;te -</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>Datum eingrenzen</td>
      <td colspan="2">von 
        <input name="datumvon" type="text" id="datumvon" value="<?php echo $HTTP_POST_VARS['datumvon']; ?>">
        <script type="text/javascript">
                    <!--
                    document.write('<a title="<?php echo $strCalendar;?>" href="javascript:openCalendar(\'<?php echo $editFormAction;?>\', \'form2\', \'datumvon\', \'date\')"><img class="calendar" src="<?php echo $pmaThemeImage; ?>./picture/b_calendar.png" alt="<?php echo $strCalendar; ?>"/></a>');
                    //-->
                    </script>
        jjjj-mm-tt bis 
        ...
        jjjj-mm-tt 
        <input <?php if (!(strcmp($HTTP_POST_VARS['erweitert'],1))) {echo "checked";} ?> name="erweitert" type="checkbox" id="erweitert" value="1">
        erweiterte Textanzeige</td>
    </tr>
  </table>
  <br>
...
<table width="900" border="0" cellpadding="0" cellspacing="1">
  ...
  if ($HTTP_POST_VARS['datumvon']<>"" ){
	  $suchetxt1=" AND fhm_ereignis.datum>='".$HTTP_POST_VARS['datumvon']."' ";
  }
   if ($HTTP_POST_VARS['datumbis']<>"" ){

	  $suchetxt2="AND fhm_ereignis.datum<='".$HTTP_POST_VARS['datumbis']."' ";
  }
   
  mysql_select_db($database_qsdatenbank, $qsdatenbank);
$query_rststatus = "SELECT fhm_ereignis.datum, fhm_ereignis.id_fhme, fhm_ereignis.id_fhm, fhm_ereignis.beschreibung, fhm_ereignis.id_fhmea, fhm_ereignisart.piclink, fhm_ereignisart.name,fhm_ereignis.beschreibung  
FROM fhm_ereignis, fhm_ereignisart WHERE fhm_ereignis.id_fhm=$id_fhm AND fhm_ereignis.lokz=0 AND fhm_ereignisart.id_fhmea=fhm_ereignis.id_fhmea $suchetxt1 $suchetxt2 
...
?></select>
	  ...
	  $test=date("Y-m-d",time())-$row_rststatus['datum'];
	  
	  if ($test>=1 && $row_rststatus['piclink']=="st2.gif"){ ?>
	  <img src="picture/b_tipp.png" width="16" height="16">
	  <?php } ?>
	  
	  
	  
	  <?php if (!(strcmp($HTTP_POST_VARS['erweitert'],1))) {
	 echo "<img src=\"picture/".$row_rststatus['piclink']."\">".$row_rststatus['name']." - ".wordwrap($row_rststatus['beschreibung'],30,"<br>",255)." <br> ".$row_rststatus['datum']." - ".$test; ?></font></td>
      <?php } else {
	 echo "<img src=\"picture/".$row_rststatus['piclink']."\">".$row_rststatus['name']." - ".substr($row_rststatus['beschreibung'],0,60)."-".$row_rststatus['datum']." - ".$test; ?></font></td>
  	  <?php } ?>
