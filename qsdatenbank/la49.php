...
if ((isset($HTTP_POST_VARS["cancel2"])) or (isset($HTTP_POST_VARS["save"])) or (isset($HTTP_POST_VARS["save1"])) or (isset($HTTP_POST_VARS["save2"])) or (isset($HTTP_POST_VARS["save3"])) or (isset($HTTP_POST_VARS["save4"])) or (isset($HTTP_POST_VARS["save5"])) or  (isset($HTTP_POST_VARS["save6"] )) or  (isset($HTTP_POST_VARS["save7"] ))or  (isset($HTTP_POST_VARS["save8"] ))or  (isset($HTTP_POST_VARS["save9"] ))  ) {
if ($HTTP_POST_VARS['eingangsdatum']==""){$HTTP_POST_VARS['eingangsdatum']==date("Y-m-d",time());}
if ($HTTP_POST_VARS['fm9datum']==""){$HTTP_POST_VARS['fm9datum']==date("Y-m-d",time());}
if ($HTTP_POST_VARS['fm10datum']==""){$HTTP_POST_VARS['fm10datum']==date("Y-m-d",time());}

  $updateSQL = sprintf("UPDATE fehlermeldungen SET fm1=%s, materialnummer=%s, reklamenge=%s, lokz=%s, fm1user=%s, fm1notes=%s, fm11notes=%s, fm12notes=%s, fm0=%s, fmnotes=%s, fm1datum=%s, fm9=%s, fm10=%s,  fm9user=%s, fm10user=%s, fm9notes=%s, fm10notes=%s, fm20=%s, fm30=%s, fmpareto=%s,fm9datum=%s,fm10datum=%s WHERE fmid=%s",
					  ...
					  GetSQLValueString($HTTP_POST_VARS['lokz'],"int"),
					   ...
					   GetSQLValueString($HTTP_POST_VARS['fm9'], "int"),
					    GetSQLValueString($HTTP_POST_VARS['fm10'], "int"),
					    GetSQLValueString($HTTP_POST_VARS['fm9user'], "int"),
						 GetSQLValueString($HTTP_POST_VARS['fm10user'], "int"),
						 GetSQLValueString($HTTP_POST_VARS['fm9notes'], "text"),
						 GetSQLValueString($HTTP_POST_VARS['fm10notes'], "text"),
						 GetSQLValueString($HTTP_POST_VARS['fm20'], "text"),
						 GetSQLValueString($HTTP_POST_VARS['fm30'], "text"),
						 GetSQLValueString($HTTP_POST_VARS['fmpareto'], "int"),
						  GetSQLValueString($HTTP_POST_VARS['fm9datum'], "date"), 
						  GetSQLValueString($HTTP_POST_VARS['fm10datum'], "date"),
...
$rsttyp = mysql_query($query_rsttyp, $qsdatenbank) or die(mysql_error()." rsttyp ");
$row_rsttyp = mysql_fetch_assoc($rsttyp);
$totalRows_rsttyp = mysql_num_rows($rsttyp);

mysql_select_db($database_qsdatenbank, $qsdatenbank);
$query_rst5 = "SELECT linkfehlerartikel.lfaid, fehler.fid, fehler.fkurz, fehler.fname, fehler.lokz, linkfehlerartikel.fid, linkfehlerartikel.artikelid, linkfehlerartikel.lokz FROM fehler, linkfehlerartikel WHERE fehler.lokz=0 AND linkfehlerartikel.lokz=0 AND linkfehlerartikel.fid=fehler.fid AND linkfehlerartikel.artikelid='$row_rst4[fmartikelid]' ORDER BY fehler.fkurz";
$rst5 = mysql_query($query_rst5, $qsdatenbank) or die(mysql_error()." rst5 ");
...
$rst6 = mysql_query($query_rst6, $qsdatenbank) or die(mysql_error()." rst6 ");
...
$rst7 = mysql_query($query_rst7, $qsdatenbank) or die(mysql_error()." rst7 ");
...
$rst42 = mysql_query($query_rst42, $qsdatenbank) or die(mysql_error()." rst42 ");
...
$rst52 = mysql_query($query_rst52, $qsdatenbank) or die(mysql_error()." rst52 ");
...
mysql_select_db($database_qsdatenbank, $qsdatenbank);
$query_rst10 = "SELECT * FROM `komponenten_asm_vorlagen` WHERE (`lokz` =0) or (`lokz` =0 )ORDER BY komponenten_asm_vorlagen.gruppe, komponenten_asm_vorlagen.name";
$rst10 = mysql_query($query_rst10, $qsdatenbank) or die(mysql_error()." rst10 ");
...
<form action="<?php echo $editFormAction; ?>" name="form1" method="POST">
  ...
  <table width="730" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr bgcolor="#FFFFFF"> 
      <td width="164">Datum </td>
      <td width="362"> <input name="fm9datum" type="text" id="fm9datum" value="<?php echo $row_rst4['fm9datum']; ?>"> 
        <script type="text/javascript">
                    <!--
                    document.write('<a title="<?php echo $strCalendar;?>" href="javascript:openCalendar(\'<?php echo $editFormAction;?>\', \'form1\', \'fm9datum\', \'date\')"><img class="calendar" src="<?php echo $pmaThemeImage; ?>./picture/b_calendar.png" alt="<?php echo $strCalendar; ?>"/></a>');
                    //-->
                    </script>
        jjjj-mm-tt erledigt: 
        <input <?php if (!(strcmp($row_rst4['fm9'],1))) {echo "checked";} ?> name="fm9" type="checkbox" id="fm9" value="1"> 
      </td>
      <td rowspan="7"> <div align="right">
          <?php  include("la4q.status.php");?>
        </div></td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td>Wer ist verantwortlich ?</td>
      <td><select name="fm9user">
          <?php
do {  
?>
          <option value="<?php echo $row_rstuser['id']?>"<?php if (!(strcmp($row_rstuser['id'], $row_rst4['fm9user']))) {echo "SELECTED";} ?>><?php echo $row_rstuser['name']?></option>
          <?php
...
?>
    ...
    <tr bgcolor="#FFFFFF"> 
      <td><p>4. Analysierte Fehlerursache</p>
        <p>&nbsp;</p>
        <p>&nbsp;</p></td>
      <td> <textarea name="fm9notes" cols="50" rows="6" id="textarea3"><?php echo $row_rst4['fm9notes']; ?></textarea> 
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td><p>5. geplante Abstell- und Korrekturma&szlig;nahme</p>
        <p>&nbsp;</p>
        <p>&nbsp;</p>
        <p>&nbsp;</p></td>
      <td> <textarea name="fm20" cols="50" rows="7" id="textarea20"><?php echo $row_rst4['fm20']; ?></textarea> 
      </td>
    </tr>
	<tr bgcolor="#FFFFFF"> 
      <td><p>6. Endg&uuml;ltige Abstell- und Korrekturma&szlig;nahme</p>
        <p>&nbsp;</p>
        <p>&nbsp;</p>
        <p>&nbsp;</p></td>
      <td> <textarea name="fm10notes" cols="50" rows="7" id="textarea3"><?php echo $row_rst4['fm10notes']; ?></textarea> 
      </td>
    </tr>
	<tr bgcolor="#FFFFFF"> 
      <td><p>7. langfristige Vorsorge</p>
        <p>&nbsp;</p>
        <p>&nbsp;</p>
        <p>&nbsp;</p></td>
      <td> <textarea name="fm30" cols="50" rows="7" id="textarea30"><?php echo $row_rst4['fm30']; ?></textarea> 
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td>8. Datum best&auml;tigte Wirksamkeit</td>
      <td> <input name="fm10datum" type="text" id="fm10datum" value="<?php echo $row_rst4['fm10datum']; ?>"> 
        <script type="text/javascript">
                    <!--
                    document.write('<a title="<?php echo $strCalendar;?>" href="javascript:openCalendar(\'<?php echo $editFormAction;?>\', \'form1\', \'fm10datum\', \'date\')"><img class="calendar" src="<?php echo $pmaThemeImage; ?>./picture/b_calendar.png" alt="<?php echo $strCalendar; ?>"/></a>');
                    //-->
                    </script>
        jjjj-mm-tt erledigt: 
        <input <?php if (!(strcmp($row_rst4['fm10'],1))) {echo "checked";} ?> name="fm10" type="checkbox" id="fm10" value="1"> 
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td>Wirksamkeit best&auml;tigt </td>
      <td><img src="picture/iconchance_16x16.gif" width="16" height="16"> <select name="fm10user">
          <?php
do {  
?>
          <option value="<?php echo $row_rstuser['id']?>"<?php if (!(strcmp($row_rstuser['id'], $row_rst4['fm10user']))) {echo "SELECTED";} ?>><?php echo $row_rstuser['name']?></option>
          <?php
} while ($row_rstuser = mysql_fetch_assoc($rstuser));
  $rows = mysql_num_rows($rstuser);
  if($rows > 0) {
...
?>
        ...
        zeige als ParetoPalette 
        <input <?php if (!(strcmp($row_rst4['fmpareto'],1))) {echo "checked";} ?> name="fmpareto" type="checkbox" id="fmpareto" value="1"></td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td><a href="pdfq3_8d.php?url_fmid=<?php echo $row_rst4['fmid']; ?>&url_user_id=<?php echo $row_rst1['id'];?>" target="_blank"><img src="picture/download1.gif" alt="8-D Report anzeigen" width="17" height="17" border="0"></a> 
        8-D-Report</td>
      <td><p> 
          <input type="submit" name="save9" value="speichern">
          <input name="hurl_fmid" type="hidden" id="hurl_fmid" value="<?php echo $row_rst4['fmid']; ?>">
        </p>
        <p><a href="pdfkv1.php?url_fmid=<?php echo $row_rst4['fmid'];?>&url_user_id=<?php echo $row_rst1['id'];?>" target="_blank"><img src="picture/download1.gif" width="17" height="17" border="0">Q1 
          Online Reklamations- und Fehlermeldung -Formular</a></p>
        <p><a href="pdfq2.php?url_fmid=<?php echo $row_rst4['fmid'];?>&url_user_id=<?php echo $row_rst1['id'];?>" target="_blank"><img src="picture/download1.gif" width="17" height="17" border="0">Q2 
          Online Reklamations- und Fehlermeldung -Formular</a></p>
        <p><a href="pdfq3.php?url_fmid=<?php echo $row_rst4['fmid'];?>&url_user_id=<?php echo $row_rst1['id'];?>" target="_blank"><img src="picture/download1.gif" width="17" height="17" border="0">Q3 
          Online Reklamations- und Fehlermeldung -Formular</a> </p></td>
    </tr>
  </table>
  <p>&nbsp;</p>
    ...
    <tr> 
      ...
      <td width="585"> <table width="595" border="0" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
          ...
          <tr bgcolor="#FFFFFF"> 
            ...
            <td>
<input <?php if (!(strcmp($row_rst4['lokz'],1))) {echo "checked";} ?> name="lokz" type="checkbox" id="lokz" value="1"></td>
          </tr>
          <tr bgcolor="#FFFFFF"> 
...
?>
              ...
              <?php } // Show if recordset empty ?> <?php if ($totalRows_rst5 > 0) { // Show if recordset not empty ?> <em><br>
              m&ouml;gliche Fehler</em><br> <table width="600" border="0" cellpadding="0" cellspacing="0">
                <?php do {?>
                <tr onmouseover="setPointer(this, 1, 'over', '#EEEEEE', '#CCCCFF', '#FFCC99');" onmouseout="setPointer(this, 1, 'out', '#EEEEEE', '#CCCCFF', '#FFCC99');" onmousedown="setPointer(this, 1, 'click', '#EEEEEE', '#CCCCFF', '#FFCC99');"> 
...
<p>
  <?php
mysql_free_result($rst1."  rst1 ");

mysql_free_result($rst3."  rst3 ");

mysql_free_result($rst2."  rst2 ");

mysql_free_result($rst4."  rst4 ");

mysql_free_result($rst5."  rst5 ");

mysql_free_result($rst6."  rst6 ");

mysql_free_result($rsttyp."  rsttyp ");

mysql_free_result($rst9."  rst9 ");

mysql_free_result($rst10."  rst10 ");
?>
</p>
