<?php require_once('Connections/qsdatenbank.php'); ?><?php $la = "la260";
...
if ($HTTP_POST_VARS['mdatum']==""){$datumaktuell = "now()";
  ...
  $updateSQL = sprintf("INSERT INTO materialbewegung (martikelid, user, mdatum, mbdiff, mbwa, lokz, mbtxt) VALUES ( %s, %s, %s, %s, %s, %s, %s)",
...
mysql_select_db($database_qsdatenbank, $qsdatenbank);
$query_rst2 = "SELECT * FROM artikeldaten WHERE artikeldaten.aktiviert=1  AND artikeldaten.artikelid='$HTTP_POST_VARS[hurl_artikelid]'";
$rst2 = mysql_query($query_rst2, $qsdatenbank) or die(mysql_error());
$row_rst2 = mysql_fetch_assoc($rst2);
$totalRows_rst2 = mysql_num_rows($rst2);

$BestandNeu = $row_rst2['Bestand']+$HTTP_POST_VARS['mbdiff'];
$updateSQL = sprintf("UPDATE artikeldaten SET Bestand=%s WHERE artikelid=%s",
                       GetSQLValueString($BestandNeu, "double"),
                       GetSQLValueString($HTTP_POST_VARS['hurl_artikelid'], "int"));
...
<form name="insertform" method="POST" action="<?php echo $editFormAction; ?>">
  ...
  <table width="830" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
    ...
    <tr> 
      <td>Datum</td>
      <td> <input type="text" name="mdatum" value="<?php echo $row_rst3['mdatum']; ?>" size="20" maxlength="99" class="textfield" onchange="return unNullify('fdatum', '0')" tabindex="10" id="mdatum" /> 
        <script type="text/javascript">
                    <!--
                    document.write('<a title="Kalender" href="javascript:openCalendar(\'lang=de-utf-8&amp;server=1&amp;collation_connection=utf8_general_ci\', \'insertform\', \'mdatum\', \'date\')"><img class="calendar" src="./themes/darkblue_orange/img/b_calendar.png" alt="Kalender"/></a>');
                    //-->
                    </script></td>
      <td>&nbsp;</td>
    </tr>
