...
/* 2. Seriennummer */
$iii=2;
			if (strlen($url_sn_bis)==1){		
			$url_nummer="000".$url_sn_bis;}
			elseif (strlen($url_sn_bis)==3){		
			$url_nummer="0".$url_sn_bis;}
			elseif (strlen($url_sn_bis)==5){		
			$url_nummer="0".$url_sn_bis;}
			else{
			$url_nummer="00".$url_sn_bis;
		}
...
$im=$e->Stroke($url_nummer,"barcode/testbild".$iii.".png");
		...
		$pdf->Text(65,5,"ab Serien-Nr.");
		$pdf->Text(85,5," bis Serien-Nr.");
		...
		$pdf->Text(2,25,"FD ".date("d-m-Y",time() ));
			$pdf->SetFont('Helvetica','',13);
		$pdf->Text(2,30,"von SN ".$url_sn_ab);
		$pdf->Text(2,35,"bis SN ".$url_sn_bis);
		
		$pdf->SetFont('Helvetica','',9);
		$pdf->ln(35);
		
		 $pdf->MultiCell(100,3,utf8_decode($row_rst4['Bezeichnung']),0,"L",0); 
